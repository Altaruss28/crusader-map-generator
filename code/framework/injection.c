...
static AddressData address_data[] = {
	[ADDR_CE_BUILDING_PLACEMENT_VALIDATION] = {0, false, "?? ?? ?? ?? 50 51 8B 4C 24 24 57"}, // 0x00516316
	[ADDR_CE_WHEAT_FARM_PLACEMENT_CHANGING_SURFACE] = {0, false, "?? 03 CF 51 50 52 8B CE E8 ?? ?? ?? ?? 83 C3 01 3B 9E ?? ?? ?? ?? 0F 8C ?? ?? ?? ?? 8B 5C 24 10"}, // 0x005150DD
	[ADDR_CE_HOP_FARM_PLACEMENT_CHANGING_SURFACE] = {0, false, "?? 03 D5 52 50 8B"}, // 0x0051536B
	[ADDR_CE_APPLE_FARM_PLACEMENT_CHANGING_SURFACE] = {0, false, "?? 03 D7 52 55 50"}, // 0x00515908
	[ADDR_CE_DAIRY_FARM_PLACEMENT_CHANGING_SURFACE] = {0, false, "?? 6A 00 6A 06 03 CB"}, // 0x00514D03
	[ADDR_CE_KEEP_MAIN_BUILDING_PLACEMENT_CHANGING_SURFACE] = {0, false, "?? ?? 8B 8E ?? ?? ?? ?? 68 80"}, // 0x00514852
	[ADDR_CE_KEEP_CAMPFIRE_PLACEMENT_CHANGING_SURFACE] = {0, false, "?? ?? 68 80 00 00 00 6A 00 52"}, // 0x00514CE7
	[ADDR_CE_KEEP_PLACEMENT_STARTING_SCENARIO_GAME] = {0, false, "?? ?? ?? ?? ?? ?? 83 F8 06 0F 84 ?? ?? ?? ?? 6A 00"}, // 0x0042118D
	[ADDR_CE_PITCH_DITCH_CHANGING_HEIGHT] = {0, false, "?? ?? 80 00 FC EB"}, // 0x005117E5
	[ADDR_CE_WALL_PLACEMENT_VALIDATION] = {0, false, "?? ?? ?? ?? ?? ?? 8B 44 24 3C 6A 04 50 B9"}, // 0x00502F73
	[ADDR_CE_WALL_COST_CHECK] = {0, false, "?? 7F 8B 15 ?? ?? ?? ?? 83"}, // 0x00457E88
	[ADDR_CE_TREE_PLACEMENT_NEXT_TO_MOST_THINGS] = {0, false, "?? 50 56 55 8D"}, // 0x00515E35
	[ADDR_CE_DEER_COUNT_RANDOMIZATION] = {0, false, "?? BF 2C 00 00 00 83"}, // 0x00525C40
	[ADDR_CE_LION_COUNT_RANDOMIZATION] = {0, false, "?? BF 2D 00 00 00 83 C0"}, // 0x00525C8F
	[ADDR_CE_RABBIT_COUNT_RANDOMIZATION] = {0, false, "?? BF 2E 00 00 00 83"}, // 0x00525CCB
	[ADDR_CE_CAMEL_COUNT_RANDOMIZATION] = {0, false, "?? BF 2F 00 00 00 83"}, // 0x00525D06
	[ADDR_TILE_STATE_PTR] = {0, true, "?? ?? ?? ?? E8 ?? ?? ?? ?? 3B 44 24 6C 89 44 24 24 7E 08"}, // 0x01A93208
	[ADDR_TR_BUILDING_AND_PITCH_DITCH_PLACEMENT_FUNCTION] = {0, false, "83 EC 08 53 55 8B 6C 24 18 56 8B F1"}, // 0x005162D0
	[ADDR_TR_WALL_PLACEMENT_FUNCTION] = {0, false, "83 EC 28 8B 54"}, // 0x00502F30
	[ADDR_TR_ROCK_PLACEMENT_FUNCTION] = {0, false, "51 8B 44 24 08 53 55"}, // 0x004FB4D0
	[ADDR_TR_ROCK_ORIENTATION_VALUE] = {0, true, "?? ?? ?? ?? 00 00 00 00 A3 ?? ?? ?? ?? A1 ?? ?? ?? ?? 83"}, // 0x01FE7A98
	[ADDR_TR_PLANT_PLACEMENT_FUNCTION] = {0, false, "83 EC 0C 53 8B 5C 24 14 81 FB 8F 01 00 00 57"}, // 0x00515DA0
	[ADDR_TR_PLANT_STAGE_VALUE] = {0, false, "?? 6A 00 0F B7 C0 6A"}, // 0x00515EA0
	[ADDR_TR_APPLE_TREE_PLACEMENT_FUNCTION] = {0, false, "8B 44 24 0C 8B 4C 24 08 6A ?? 6A 00"}, // 0x004F3560
	[ADDR_TR_APPLE_TREE_STAGE_VALUE] = {0, false, "?? 6A 00 6A 00 6A 01 6A 0F"}, // 0x004F3569
	[ADDR_TR_PLANT_ARRAY] = {0, true, "?? ?? ?? ?? 68 9C 00 00 00 51 B9"}, // 0x00F2CC54
	[ADDR_TR_UNIT_FUNCTION] = {0, false, "53 8B D9 B9 01"}, // 0x0053E440
	[ADDR_TR_UNIT_STATE_PTR] = {0, true, "?? ?? ?? ?? E8 ?? ?? ?? ?? 69 F6 90 04 00 00 0F BF B6"}, // 0x01387F38
	[ADDR_TR_ANIMAL_FUNCTION] = {0, false, "83 EC 08 55 57 33"}, // 0x00525BA0
	[ADDR_TR_TRIBE_STATE_PTR] = {0, true, "?? ?? ?? ?? 89 7C 24 20 E8 ?? ?? ?? ?? 8B E8"}, // 0x01667F78
	[ADDR_TR_ANIMAL_DEER_COUNT_VALUE] = {0, false, "?? 89 44 24 10 33 DB 39"}, // 0x00525C48
	[ADDR_TR_ANIMAL_LION_COUNT_VALUE] = {0, false, "?? EB AF 81 FB DE"}, // 0x00525C97
	[ADDR_TR_ANIMAL_RABBIT_COUNT_VALUE] = {0, false, "?? E9 70 FF FF FF 81 FB DF"}, // 0x00525CD3
	[ADDR_TR_ANIMAL_CAMEL_COUNT_VALUE] = {0, false, "?? E9 35 FF FF FF 81 FB"}, // 0x00525D0E
	[ADDR_IN_BASE_HEIGHT_ARRAY] = {0, true, "?? ?? ?? ?? 3B D1 0F 8D 02"}, // 0x01D46648
	[ADDR_IN_TOTAL_HEIGHT_ARRAY] = {0, true, "?? ?? ?? ?? 0F 8D DC 03"}, // 0x01D32C38
	[ADDR_IN_SURFACE_ARRAY] = {0, true, "?? ?? ?? ?? 75 11 83 7C 24 18"}, // 0x01C471E8
	[ADDR_IN_LOGICAL_ARRAY] = {0, true, "?? ?? ?? ?? 00 00 00 10 89 74 24 18"}, // 0x01BF8368
	[ADDR_IN_CLEAR_MAP_FUNCTION] = {0, false, "53 55 56 8B F1 57 33 FF 89 BE"}, // 0x00512100
	[ADDR_IN_MINIMAP_STATE_PTR] = {0, true, "?? ?? ?? ?? E8 ?? ?? ?? ?? 83 6C 24 10 01 68"}, // 0x01A31610
	[ADDR_IN_MINIMAP_FIX_FUNCTION_FIRST] = {0, false, "55 8B EC 81 EC 18 01 00 00 53"}, // 0x00509180
	[ADDR_IN_MINIMAP_FIX_FUNCTION_SECOND] = {0, false, "81 3D ?? ?? ?? ?? C8 00 00 00 8B 44 24 08 8B 54 24 04 50 52 7E 0E 6A 01 6A 02 6A 04 E8 ?? ?? ?? ?? C2 08 00 6A 02 6A 04 6A 04 E8 ?? ?? ?? ?? C2 08 00 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 55"}, // 0x004B6680
	[ADDR_IN_MINIMAP_FIX_FUNCTION_THIRD] = {0, false, "81 3D ?? ?? ?? ?? C8 00 00 00 8B 44 24 08 8B 54 24 04 50 52 7E 0E 6A 01 6A 02 6A 04 E8 ?? ?? ?? ?? C2 08 00 6A 02 6A 04 6A 04 E8 ?? ?? ?? ?? C2 08 00 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 81"}, // 0x004B6640
	[ADDR_IN_TEXT_BUFFER_BASE] = {0, true, "?? ?? ?? ?? E8 ?? ?? ?? ?? 6A 06 B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? BE"}, // 0x01652740 + 0x344 = 0x01652A84
	[ADDR_IN_MAP_DESCRIPTION_BUFFER] = {0, true, "?? ?? ?? ?? EB 25 A1 ?? ?? ?? ?? 3B C6"}, // 0x01FE8680
	[ADDR_IN_SAVE_MAP_FUNCTION] = {0, false, "81 EC F4 03 00 00 A1 ?? ?? ?? ?? 33 C4 89 84 24 F0 03 00 00 A1 ?? ?? ?? ?? 53"}, // 0x004950B0
	[ADDR_IN_MAP_TYPE_VALUE] = {0, true, "?? ?? ?? ?? 74 46 57 33 FF 89"}, // 0x01FE9244
	[ADDR_IN_MENU_ACTION_VALUE] = {0, true, "?? ?? ?? ?? 09 00 00 00 E8 ?? ?? ?? ?? 6A 00"}, // 0x01126600
	[ADDR_IN_MENU_VIEW_VALUE] = {0, true, "?? ?? ?? ?? 10 75 37 83 3D"}, // 0x01FE7D1C
};
