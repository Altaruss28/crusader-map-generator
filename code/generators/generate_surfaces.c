...
bool generate_surfaces(Map *map, Config *config, u32 *rng_state, DynamicString *logs)
{
	...
	i32 river_height_offset = config->surface_river_height_offset;
	u32 river_ford_transform_chance = config->surface_river_ford_transform_chance;
	
	u32 river_share = config->surface_river_share;
	u32 grass_dark_share = config->surface_grass_dark_share;
	u32 grass_medium_share = config->surface_grass_medium_share;
	u32 grass_light_share = config->surface_grass_light_share;
	u32 earth_and_stones_share = config->surface_earth_and_stones_share;
	u32 earth_share = config->surface_earth_share;
	u32 surface_share_count = river_share + grass_dark_share + grass_medium_share + grass_light_share + earth_and_stones_share + earth_share;
	...
	if (cores_placed < core_count) {
		if (!add_to_string(logs, "generate_surfaces: %d/%d surface cores placed\n", cores_placed, core_count)) goto out;
	}
	...
	u32 river_count = river_share * tiles_per_share;
	...
	u32 earth_count = claimed_tiles->usage - (river_count + grass_dark_count + grass_medium_count + grass_light_count + earth_and_stones_count);
	...
	for (i32 tile_index = claimed_tiles->usage - 1; tile_index >= 0; tile_index--) {
		...
		if (surface == SURFACE_RIVER) {
			set_feature(map, x, y, FEATURE_RIVER);
			set_height(map, x, y, tm[x][y].height + river_height_offset);
		} else if (surface == SURFACE_GRASS_DARK || surface == SURFACE_GRASS_MEDIUM || surface == SURFACE_GRASS_LIGHT) {
			set_feature(map, x, y, FEATURE_GRASSLAND);
		}
	...
	for (u32 x_origin = 0; x_origin < MAP_SIZE; x_origin++) {
		for (u32 y_origin = 0; y_origin < MAP_SIZE; y_origin++) {
			...
			|| tm[x_origin][y_origin].surface != SURFACE_RIVER) continue;
			...
			for (u32 offset_index = 0; offset_index < 8; offset_index++) {
				...
				if (feature == FEATURE_RIVER) continue;
				
				next_to_non_river = true;
				
				if (feature != FEATURE_RAMP) continue;
			...
			if (!next_to_ramp
			&& (next_to_non_river || river_ford_transform_chance < (u32)random(rng_state, 1, 100))) continue;
			
			set_surface(map, x_origin, y_origin, SURFACE_FORD);
	...
	for (u32 x_origin = 0; x_origin < MAP_SIZE; x_origin++) {
		for (u32 y_origin = 0; y_origin < MAP_SIZE; y_origin++) {
			...
			if (!special_surface_found) continue;
